...
export default class Blog extends Emitter {
   ...
   */
  async post(id: number, slug: string) {
    const { post } = await this._fetch(gql.queries.post, { id, slug })
    return post
  }
   ...
   * @returns Object<status, message>
   */
  async posts(limit: number, page: number, ids?: [number]) {
    const { posts } = await this._fetch(gql.queries.posts, { limit, page, ids })
    return posts
  }
   ...
   * @param count
   */
  async popularTopics(count: number = 8) {
