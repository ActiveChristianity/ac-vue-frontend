        ...
        <g-link :to="post.slug" :key="post.id" class="flex items-start p-4 rounded-2xl transition-shadow duration-300 hover:shadow-outline">
          ...
          <div class="pl-4">
            ...
            <p class="text-gray-600 text-sm mt-2">
              <span aria-label="time2read"><i class="text-md mr-1 fal fa-file-alt"></i> {{ Math.ceil(post.readtime / 60) }} mins</span>
              <span aria-label="views" class="ml-2"><i class="text-md mr-1 fal fa-eye"></i> {{ post.views}}</span>
            </p>
          </div>
        </g-link>
      </template>
