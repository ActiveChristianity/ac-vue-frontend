<template>
  <header class="bg-white pb-12">
    ...
    <div id="headbar"
      class="fixed z-30 w-full bg-white bg-white flex items-center justify-between border-b border-gray-200 pl-1 pr-4 md:px-4 h-12"
      :style="{transform: $store.showSearch ? 'translateY(0)' : `translateY(${top}px)`}"
    ...
    >
      ...
      <div v-if="! $store.showSearch" class="hidden md:flex flex-1">
        <g-link :to="`/${$t.slug_topic}`" :title="$t.topics" class="text-gray-800 tracking-wide rounded m-1 py-1 px-2 hover:bg-gray-200">{{ $t.topics }}</g-link>
        <g-link :to="`/${$t.slug_glossary}`" :title="$t.glossary" class="text-gray-800 tracking-wide rounded m-1 py-1 px-2 hover:bg-gray-200">{{ $t.glossary }}</g-link>
        <g-link :to="`/${$t.slug_about}`" title="About Us" class="text-gray-800 tracking-wide rounded m-1 py-1 px-2 hover:bg-gray-200">{{ $t.about }}</g-link>
      </div>
